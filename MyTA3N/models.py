...
from model.MultiHeadAttention import PositionalEncoder
...
class VideoModel(nn.Module):
	def __init__(self, num_class, baseline_type, frame_aggregation, modality,
				...
				share_params='Y',use_frame_self_attention=True,use_relation_self_attention=False,
				 augmented_inputs=True):
		...
		self.use_frame_self_attention = use_frame_self_attention
		self.use_relation_self_attention = use_relation_self_attention
	...
	def _prepare_DA(self, num_class, base_model, modality): # convert the model to DA framework
		...
		if self.use_frame_self_attention:
			# Add positional encoding
			self.positional_encoder = PositionalEncoder(input_dimension=self.feature_dim)
			self.frame_encoder_layer = nn.TransformerEncoderLayer(d_model=self.feature_dim,
																  nhead=2,dim_feedforward=feat_shared_dim,
																  dropout=0.1)
			self.frame_encoder = nn.TransformerEncoder(encoder_layer=self.frame_encoder_layer,num_layers=1)
		...
		if self.frame_aggregation == 'rnn': # 2. rnn
			...
			if self.use_relation_self_attention:
				self.relation_positional_encoder = PositionalEncoder(input_dimension=self.num_bottleneck)
				self.relation_frame_encoder_layer = nn.TransformerEncoderLayer(d_model=self.num_bottleneck,
																	  nhead=2, dim_feedforward=self.num_bottleneck,
																	  dropout=0.1)
				self.relation_frame_encoder = nn.TransformerEncoder(encoder_layer=self.relation_frame_encoder_layer, num_layers=1)
		...
		elif self.frame_aggregation=="Transformer":
			#todo : implement there the transformer method
			pass

		# ------ video-level layers (source layers + domain layers) ------#
		if self.frame_aggregation == 'avgpool': # 1. avgpool
			if self.use_relation_self_attention:
				self.relation_positional_encoder = PositionalEncoder(input_dimension=feat_shared_dim)
				self.relation_frame_encoder_layer = nn.TransformerEncoderLayer(d_model=feat_shared_dim,
																	  nhead=2, dim_feedforward=feat_shared_dim,
																	  dropout=0.1)
				self.relation_frame_encoder = nn.TransformerEncoder(encoder_layer=self.relation_frame_encoder_layer, num_layers=1)
	...
	def aggregate_frames(self, feat_fc, num_segments, pred_domain):
		...
		if self.frame_aggregation == 'rnn':
			...
			elif self.use_attn == "Transformer":
				#todo : Add here a transformer
	...
	def forward(self, input_source, input_target, beta, mu, is_train, reverse):
		r""" Implement the forward method

		Args:
			input_source (torch.Tensor) : The input from the source set. Accepted dimension is (batch_size,num_segments,input_size)
			input_target (torch.Tensor) : Input from target set. Accepted dimension is (batch_size,num_segments,input_size)
			beta (list): List having the beta parameter of the gradient reversal layer of each of the domain adaptation.
				Frame-level,relation-level,video-level. By default it can be set up to [1,1,1]
			mu (int): List having the mu parameter of the gradient reversal layer. By default set it up to 0
			is_train (boolean) : Boolean indicating if the call is during training. It changes the attention part
			reverse(boolean) : Boolean indicating if the call is to be using the GRL.
		"""
		batch_source = input_source.size()[0] # Retrieve the batch size of source input
		batch_target = input_target.size()[0] # Retrieve bacth size of targ input
		num_segments = self.train_segments if is_train else 5 #fixme : put again self.val_segments
		# sample_len = (3 if self.modality == "RGB" else 2) * self.new_length
		sample_len = self.new_length
		feat_all_source = [] # List containing the features obtained at each step of the network (after first fc, after temporal aggregation,...)
		feat_all_target = []
		pred_domain_all_source = []
		pred_domain_all_target = []
		if self.use_frame_self_attention:
			#Add positional encoding
			input_source = self.positional_encoder(input_source)
			input_target = self.positional_encoder(input_target)
			# Pass through the encoder
			input_source = self.frame_encoder(input_source)
			input_target = self.frame_encoder(input_target)
		...
		feat_fc_source = self.fc_feature_shared_source(feat_base_source) #(batch_size*num_segments,min(self.fc_dim, self.feature_dim))
		...
		# Apply ReLu and dropout after first fully connected layer
		...
		pred_fc_domain_frame_source = self.domain_classifier_frame(feat_fc_source, beta) # prediction of domain (batch_size*num_segments,2)
		pred_fc_domain_frame_target = self.domain_classifier_frame(feat_fc_target, beta)
		# Add predictions to corresponding lists
		...
		if self.frame_aggregation == 'avgpool' or self.frame_aggregation == 'rnn':
			if self.use_relation_self_attention:
				...
				# Add positional encoding
				feat_fc_video_source = self.relation_positional_encoder(feat_fc_video_source)
				feat_fc_video_target = self.relation_positional_encoder(feat_fc_video_target)
				# Pass through the encoder
				feat_fc_video_source = self.relation_frame_encoder(feat_fc_video_source)
				feat_fc_video_target = self.relation_frame_encoder(feat_fc_video_target)
				# Go again to size
				feat_fc_source = feat_fc_video_source.view(-1,1024)
				feat_fc_target = feat_fc_video_target.view(-1,1024)

			feat_fc_video_source = self.aggregate_frames(feat_fc_source, num_segments, pred_fc_domain_frame_source) # (batch_size,feat_shared_size)
			...
			#actually output is 128x4x256
			if self.use_relation_self_attention:
				# Add positional encoding
				feat_fc_video_relation_source = self.relation_positional_encoder(feat_fc_video_relation_source)
				feat_fc_video_relation_target = self.relation_positional_encoder(feat_fc_video_relation_target)
				# Pass through the encoder
				feat_fc_video_relation_source = self.relation_frame_encoder(feat_fc_video_relation_source)
				feat_fc_video_relation_target = self.relation_frame_encoder(feat_fc_video_relation_target)
