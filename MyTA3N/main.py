...
def main():
    ...
    model = VideoModel(num_class, args.baseline_type, args.frame_aggregation, args.modality,
                       ...
                       use_frame_self_attention=args.use_frame_self_attention,
                       use_relation_self_attention=args.use_relation_self_attention,
                       augmented_inputs=args.augmented_inputs)

    model = model.cuda()  # torch.nn.DataParallel(model, args.gpus).cuda()
    ...
    if args.optimizer == 'SGD':
        ...
        optimizer = torch.optim.SGD([{'params': transformer_parameters}, {'params': other_parameters}], args.lr,
                                    momentum=args.momentum, weight_decay=args.weight_decay,
                                    nesterov=True)
    elif args.optimizer == 'Adam':
        print(Fore.YELLOW + 'using Adam')
        optimizer = torch.optim.Adam([{'params': transformer_parameters}, {'params': other_parameters}], args.lr,
                                     weight_decay=args.weight_decay)
    else:
        print(Back.RED + 'optimizer not support or specified!!!')
        exit()
    # have different learning rates
    optimizer.param_groups[0]['lr'] = args.lr2  # Transformer parameters
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if epoch % args.eval_freq == 0 or epoch == args.epochs:
            if target_set.labels_available:
                prec1_val, prec1_verb_val, prec1_noun_val = validate(target_test_set_loader,
                                                                     model, criterion, num_class, epoch,
                                                                     val_file, None)  # fixme : Include writer_val
...
def train(num_class, source_loader, target_loader, model, criterion, criterion_domain, optimizer, epoch, log, log_short,
    ...
    if args.no_partialbn:
        pass
        # model.module.partialBN(False) fixme : put again
    else:
    ...
    pre_processor = PreProcessor(input_size=2048)
    ...
    for i, ((source_data, source_label, source_id), (target_data, target_label, target_id)) in data_loader:
        ...
        if batch_source_ori < args.batch_size[0]:
            source_data_dummy = torch.zeros(args.batch_size[0] - batch_source_ori, source_size_ori[1],
                                            source_size_ori[2]).to("cuda")
            source_data = torch.cat((source_data, source_data_dummy))
            for i, sl in enumerate(source_label):
                source_labels_dummy = torch.zeros(args.batch_size[0] - batch_source_ori, 1).long()
                source_label[i] = torch.cat((sl, source_labels_dummy))

        if batch_target_ori < args.batch_size[1]:
            target_data_dummy = torch.zeros(args.batch_size[1] - batch_target_ori, target_size_ori[1],
                                            target_size_ori[2]).to("cuda")
            target_data = torch.cat((target_data, target_data_dummy))
            for i, sl in enumerate(target_label):
                target_labels_dummy = torch.zeros(args.batch_size[1] - batch_target_ori, 1).long()
                target_label[i] = torch.cat((target_label[i], target_labels_dummy))
        ...
        source_label_verb = torch.reshape(source_label[0].cuda(non_blocking=True),
                                          (args.batch_size[0],))  # pytorch 0.4.X
        source_label_noun = torch.reshape(source_label[1].cuda(non_blocking=True),
                                          (args.batch_size[0],))  # pytorch 0.4.X

        target_label_verb = torch.reshape(target_label[0].cuda(non_blocking=True),
                                          (args.batch_size[1],))  # pytorch 0.4.X
        target_label_noun = torch.reshape(target_label[1].cuda(non_blocking=True),
                                          (args.batch_size[1],))  # pytorch 0.4.X
        ...
        label_verb = label_verb[:batch_source_ori]
        label_noun = label_noun[:batch_source_ori]

        label_verb = torch.reshape(label_verb, (batch_source_ori,))
        label_noun = torch.reshape(label_noun, (batch_source_ori,))
        ...
        if args.adv_DA != 'none' and args.use_target != 'none':
            loss_adversarial = torch.tensor(0, dtype=torch.float32).to("cuda")
            pred_domain_all = []
        ...
        if args.use_loss_mcc != "none":
            #COmpute MCC loss using ONLY verb loss. In future it could be implemented using verb and noun
            mu_mcc=1
            loss_mcc = MCCLoss(out_target[0])
            loss += mu_mcc*loss_mcc
        ...
        pred_verb = out_verb
        # todo : make work with 5 in top k. Modify to have topk 5 . delete no necessary anymore
        prec1_verb, prec5_verb = accuracy(pred_verb.data, label_verb)[0], accuracy(pred_verb.data, label_verb)[
            0]  # topk=(1, 5))
        pred_noun = out_noun
        prec1_noun, prec5_noun = accuracy(pred_noun.data, label_noun)[0], accuracy(pred_noun.data, label_noun)[
            0]  # topk=(1, 5))
        ...
        if i % args.print_freq == 0:
            line = 'Train: [{0}][{1}/{2}], lr: {lr:.5f}\t' + \
                   ...
                   'PrecVerb@1 {top1_verb.val:.3f} ({top1_verb.avg:.3f})\t' + \
...
def validate(val_loader, model, criterion, num_class, epoch, log, tensor_writer=None):
    global args
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1_verb = AverageMeter()
    ...
    for i, (val_data, val_label, _) in enumerate(val_loader):
        if args.augmented_inputs:
            val_data, _, _ = pre_processor(val_data)
        val_size_ori = val_data.size()  # original shape
        batch_val_ori = val_size_ori[0]

        val_data = val_data.to("cuda")
        val_label[0], val_label[1] = val_label[0].to("cuda"), val_label[1].to("cuda")
        ...
        if batch_val_ori < args.batch_size[2]:
            ...
            target_data_dummy = torch.zeros(args.batch_size[2] - batch_val_ori, val_size_ori[1],
                                            val_size_ori[2]).to("cuda")
            val_data = torch.cat((val_data, target_data_dummy))
            for i, sl in enumerate(val_label):
                target_labels_dummy = torch.zeros(args.batch_size[2] - batch_val_ori, 1).long().to("cuda")
                val_label[i] = torch.cat((val_label[i], target_labels_dummy))
        ...
        with torch.no_grad():
            ...
            # compute output
            # val_data = torch.transpose(val_data,-2,-1)
            ...
            label_verb = label_verb[:batch_val_ori]
            label_noun = label_noun[:batch_val_ori]

            label_verb = torch.reshape(label_verb, (batch_val_ori,))
            label_noun = torch.reshape(label_noun, (batch_val_ori,))
